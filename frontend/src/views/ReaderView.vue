        ...
        <div class="flex items-center justify-between max-w-6xl mx-auto">
          <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center space-x-4 text-sm text-gray-300">
              ...
              <span v-if="totalPages > 0">进度: {{ (currentPage / totalPages * 100).toFixed(1) }}%</span>
              <span class="hidden sm:inline">{{ readingMode === 'single' ? '单页模式' : '双页模式' }}</span>
    ...
    >
      ...
      <div class="absolute inset-0 flex">
        ...
        >
          <!-- 导航提示 -->
          <div 
            v-if="(pageDirection === 'ltr' && navHint === 'prev') || (pageDirection === 'rtl' && navHint === 'next')" 
        ...
        >
          <!-- 导航提示 -->
          <div 
            v-if="(pageDirection === 'ltr' && navHint === 'next') || (pageDirection === 'rtl' && navHint === 'prev')" 
      ...
      <!-- 漫画图片 -->
      <div class="absolute inset-0 flex justify-center items-center p-2 overflow-hidden">
        <div v-if="error" class="text-red-400 text-xl">加载失败: {{ error }}</div>
        ...
        >
          <div :key="`content-${pageTransitionKey}`" class="w-full h-full flex justify-center items-center">
            <!-- 单页模式 -->
            <img
      ...
      >
        ...
        <div class="relative">
          ...
          >
            ...
            >
              <div class="bg-black/90 backdrop-blur-md border border-white/20 rounded-lg p-3 shadow-2xl">
                <div class="flex flex-col items-center space-y-3">
                  ...
                  <div class="text-xs text-white text-center">
                    ...
                    <div class="text-gray-300">{{ ((progressPreviewPage / totalPages) * 100).toFixed(1) }}%</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="flex items-center justify-between max-w-6xl mx-auto">
          ...
          <div class="flex-1 mx-8 text-center">
            <div class="text-sm text-gray-300">
              {{ currentPage }} / {{ totalPages }} · {{ (currentPage / totalPages * 100).toFixed(1) }}%
            </div>
          </div>
          ...
          <div class="flex items-center space-x-3">
            ...
            >
              <svg v-if="readingMode === 'single'" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const loadCurrentPage = async (isUserNavigation = false) => {
  ...
  try {
    ...
    if (readingMode.value === 'double' && currentPage.value < totalPages.value) {
      try {
        console.log('Loading next page for double mode:', currentPage.value + 1)
        nextPageUrlResult = preloadedUrls.value.get(currentPage.value + 1)
        if (!nextPageUrlResult) {
          nextPageUrlResult = await getArchivePage(archiveId.value, currentPage.value + 1)
        } else {
          console.log('Using preloaded URL for next page:', currentPage.value + 1)
        }
...
// 监听进度数据变化，恢复阅读位置
watch(progressData, (newProgress, oldProgress) => {
  ...
  if (!oldProgress && newProgress) {
    ...
    console.log('Progress data updated, but not changing currentPage to avoid loop')
  }
}, { immediate: true })
...
const preloadPages = async () => {
  ...
  for (const { page: pageNum, priority } of sortedPages) {
    if (!preloadedPages.value.has(pageNum)) {
      ...
      try {
        console.log(`Starting ${priority} priority preload for page ${pageNum}`)
...
watch(currentPage, (newPage, oldPage) => {
  ...
  if (currentPage.value > 0) {
    ...
    const isUserNavigation = oldPage !== undefined && oldPage !== newPage && !isLoading.value
...
const prevPage = () => {
  ...
  if (currentPage.value > 1) {
    ...
    pageTransitionName.value = pageDirection.value === 'ltr' ? 'slide-right' : 'slide-left'
...
const nextPage = () => {
  ...
  if (currentPage.value < totalPages.value) {
    ...
    pageTransitionName.value = pageDirection.value === 'ltr' ? 'slide-left' : 'slide-right'
    if (pageAnimationEnabled.value) {
      triggerPageTransition()
    }
...
const setToolbarTimer = () => {
  ...
  if (autoHideUI.value) {
    ...
    toolbarTimer.value = setTimeout(() => {
      console.log('Toolbar timer triggered - showSettings:', showSettings.value, 'isHoveringProgress:', isHoveringProgress.value, 'isDraggingProgress:', isDraggingProgress.value)
...
const handleProgressHover = (event: MouseEvent) => {
  ...
  const previewPage = Math.max(1, Math.min(totalPages.value, Math.round(percent * totalPages.value)))
...
const handleImageLoad = (event: Event) => {
  ...
  if (readingMode.value === 'single') {
    ...
    const nextLoaded = currentPage.value >= totalPages.value || loadedImages.value.has(currentPage.value + 1)
...
const handleVisibilityChange = () => {
  if (document.hidden) {
    ...
    if (saveProgressTimer.value) {
      ...
      // 直接保存最新进度
      if (archiveId.value && (pendingProgressPage.value || currentPage.value > 0)) {
...
.slider-glass::-webkit-slider-thumb:hover {
  background: linear-gradient(145deg, rgba(37, 99, 235, 0.95), rgba(29, 78, 216, 1));
  height: 24px;
  width: 24px;
  box-shadow: 

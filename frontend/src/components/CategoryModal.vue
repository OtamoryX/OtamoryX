<template>
  ...
  >
    <div class="space-y-6">
      ...
      <!-- 分类类型显示（编辑时显示） -->
      <div v-if="isEditing" class="bg-white/5 p-4 rounded-lg border border-white/10">
        <div class="flex items-center space-x-3">
          <div :class="[
            'p-2 rounded-lg',
            isStatic ? 'bg-purple-500/20 text-purple-300' : 'bg-green-500/20 text-green-300'
          ]">
            <svg v-if="isStatic" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <!-- 动态分类的搜索条件 -->
      <div v-if="!isEditing && categoryType === 'dynamic'" class="space-y-4">
      ...
      <div class="flex justify-between">
        ...
        <div class="flex space-x-3">
          ...
          >
            ...
            {{ isLoading ? (isEditing ? '保存中...' : '创建中...') : (isEditing ? '保存' : '创建') }}
          </button>
        </div>
      </div>
    </template>
...
import { createCategory, createDynamicCategory, updateCategory, deleteCategory } from '@/utils/api'
import type { 
  Category, 
  DynamicCategory, 

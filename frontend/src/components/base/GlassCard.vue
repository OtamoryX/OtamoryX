<template>
  ...
  >
    <!-- 装饰性光泽效果 -->
    <div v-if="shineEffect" class="glass-shine absolute inset-0 opacity-20"></div>
    
    <!-- 内容插槽 -->
    <slot />
    
    <!-- 底部模糊边框 -->
    <div v-if="bottomBorder" class="absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-white/30 to-transparent"></div>
  </div>
</template>

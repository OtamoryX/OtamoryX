<template>
  <button class="w-full text-left border border-[var(--border)] bg-[var(--bg-card)] hover:border-[var(--accent)] transition-colors rounded p-3 flex gap-3" @click="emit('open', group)">
...
const props = defineProps<{ group: VersionGroup }>()
const emit = defineEmits<{ open: [group: VersionGroup] }>()

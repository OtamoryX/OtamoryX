  ...
  <div class="directory-browser">
    ...
    >
      ...
      >
        ...
        <div class="absolute top-0 left-0 w-32 h-32 bg-blue-500/10 rounded-full blur-xl"></div>
        <div class="absolute bottom-0 right-0 w-24 h-24 bg-purple-500/10 rounded-full blur-lg"></div>
        <!-- 标题栏 -->
        <div class="relative z-10 flex items-center justify-between p-4 border-b border-white/20">
          ...
          <button
            @click="closeModal"
            class="text-white/60 hover:text-white hover:bg-white/10 p-2 rounded-lg transition-all duration-200"
          >
        ...
        <div class="relative z-10 px-4 py-3 bg-white/5 border-b border-white/20">
          <div class="flex items-center space-x-2 text-sm text-white/80">
            ...
            <span class="font-mono bg-white/10 backdrop-blur-sm px-3 py-1 rounded-lg border border-white/20 text-white">{{ currentPath || '/' }}</span>
          </div>
        </div>
        ...
        <div class="relative z-10 flex-1 overflow-y-auto p-4 min-h-0">
          ...
          <div v-else class="space-y-1">
            ...
            >
              <svg class="w-5 h-5 text-white/60 group-hover:text-blue-400 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            >
              <button
                ...
                :class="{
                  'hover:bg-white/10 cursor-pointer border-transparent hover:border-white/20': directory.is_accessible,
                  'opacity-50 cursor-not-allowed border-white/10': !directory.is_accessible,
                  'bg-blue-500/20 border-blue-400/30': selectedPath === directory.path
                }"
              >
                <svg 
                  class="w-5 h-5 mr-3"
        ...
        <!-- 底部操作栏 -->
        <div class="relative z-10 flex items-center justify-between p-4 border-t border-white/20 bg-white/5 backdrop-blur-sm">
          <div class="flex items-center space-x-2 text-sm text-white/80">
            <span>已选择:</span>
            <span class="font-mono bg-white/10 backdrop-blur-sm px-3 py-1 rounded-lg border border-white/20 max-w-md truncate text-white">
              {{ selectedPath || '未选择' }}
            </span>
          </div>

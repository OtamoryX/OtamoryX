...
**最后更新**: 2026-08-16
**当前状态**: 主要交付文档与运行入口已按仓库现状校准；已进入 Phase 8 P8-1 行为数据基础垂直切片，Phase 7 Runtime 未完成项与剩余 P1 暂后置
...
2. **Sprint 1：Phase 8 P8-1 行为数据基础** 🟡（首个垂直切片已落地）
   ...
   - 下一步补齐真实回收站移动、恢复接口和随机页“删除并显示下一本”交互。
...
- 🔜 下一步聚焦 P8-1 剩余项：真实回收站移动与恢复、删除后继续随机浏览、事件查询的管理端入口。
...
- ✅ `cargo check` 通过。
- ✅ `RUSTC_WRAPPER= cargo test --lib` 通过（80 passed / 0 failed）。
...
- 行为事件、档案处置和回收站基础表已建立；当前回收站表仅完成数据基础，真实文件移动/恢复流程尚未接入。
...
- 🔜 继续记录继续阅读、重复阅读、自动删除、恢复和规则纠正，并接入真实回收站移动/恢复。
- 🔜 优化随机页“删除并显示下一本”操作：无确认弹窗、短暂撤销。

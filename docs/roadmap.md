...
**最后更新**: 2026-04-13
**当前状态**: 已切换到“文档/脚本校准 + 业务功能收口”主线；Phase 7 插件系统真实运行时闭环为当前第一优先级，用户管理增强与剩余 P1 收尾暂后置
...
**总优先级**
1. **Sprint 0：文档与脚本校准**
   - 对齐 README、开发/部署文档、Docker Compose、历史验证脚本与仓库当前真实状态。
   - 目标：消除端口、健康检查、桌面端状态、验证入口等交付漂移，降低后续协作成本。
2. **Sprint 1：P7-3 + P7-4**
   - 推进插件 Runtime 真实链路和 Host API 收口。
   - 目标：让外部插件从“框架已建”进入“真实可执行”。
3. **Sprint 2：P7-5 + P7-6**
   - 打通执行结果回写、失败隔离、事件与调度闭环。
   - 目标：形成可观测、可恢复、可批量运行的插件执行体系。
4. **Sprint 3：Phase 8 业务闭环**
   - 在插件 runtime 站稳后，继续推进 AI 任务编排、自动标签生成与审核闭环。
   - 目标：把现有 AI 设置页从“配置能力”推进到“真实业务能力”。
5. **Sprint 4：P7-9 + 文档/测试补强**
   - 收口开发者体验、模板、分发规范和回归测试。
   - 目标：为插件生态扩展做准备。
6. **Sprint 5：剩余 P1 与用户管理增强**
...
**交付校准**
- README / 开发文档 / 部署文档 / Docker Compose / 历史测试脚本按仓库当前真实状态对齐。
- 修正文档中的端口、健康检查、桌面端现状、Node 版本与运行入口描述。

**业务功能主线**
- Phase 7：优先完成插件 Runtime、Host Callback、执行结果回写、事件与调度链路。
- Phase 8：在插件主链路可用后，推进 AI 处理编排、自动标签与人工审核闭环。
...
**当前建议起手项（推荐直接开工）**
**当前进展（2026-04-13）**
- ✅ 文档/脚本校准已提上主线：优先统一端口、健康检查、桌面端现状和验证入口。
- 🔜 下一步聚焦 `P7-3`：动态库扫描/加载、符号绑定、真实 FFI 调用闭环。
- 🔜 随后跟进 `P7-4`：`db_query` 接入共享执行器，收口 Host API。
...
- P7-9（开发者体验与分发）：⚪ 尚未开始。
- 工程健康快照：backend `cargo check` 可通过；frontend 可完成构建，当前需注意 Node 版本需满足 Vite 要求。

...
ALTER TABLE random_recommendation_sessions
    ADD COLUMN semantic_weight DOUBLE PRECISION;
ALTER TABLE random_recommendation_sessions
...
ALTER TABLE random_recommendation_items
    ADD COLUMN semantic_base_weight DOUBLE PRECISION;
ALTER TABLE random_recommendation_items
    ADD COLUMN semantic_bonus DOUBLE PRECISION NOT NULL DEFAULT 0;

CREATE TABLE IF NOT EXISTS semantic_transfer_policy (
    id INTEGER PRIMARY KEY CHECK (id = 1),
    weight DOUBLE PRECISION NOT NULL DEFAULT 0.05 CHECK (weight >= 0.0 AND weight <= 0.3),

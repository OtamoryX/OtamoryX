...
impl ImageProcessor {
    ...
    pub fn generate_thumbnail(&self, image_data: &[u8], max_size: u32) -> Result<Vec<u8>> {
        ...
        thumbnail.write_to(&mut cursor, ImageOutputFormat::Jpeg(85))
            .context("Failed to encode thumbnail")?;
        
        debug!("Generated thumbnail: {} bytes -> {} bytes", image_data.len(), output.len());
        Ok(output)
    }
    ...
    pub fn resize_image(&self, img: DynamicImage, max_size: u32) -> Result<DynamicImage> {
        ...
        debug!("Resizing image: {}x{} -> {}x{}", width, height, new_width, new_height);
